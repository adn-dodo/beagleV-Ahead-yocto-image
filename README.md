...
## 22. Investigating `u-boot-with-spl.bin`

I then inspected the generated U-Boot files to understand the bootloader
image.

The generated sizes were:

``` text
u-boot-spl.bin       144352 bytes
u-boot.bin           752568 bytes
u-boot-with-spl.bin  945080 bytes
```

The U-Boot configuration contained:
...
I converted the configured sector to a byte offset:

``` text
0x178 × 512 = 0x2F000 = 192512 bytes
```

Then I checked whether `u-boot.bin` actually begins at that location
inside `u-boot-with-spl.bin`:

``` bash
dd if=u-boot-with-spl.bin bs=1 skip=192512 status=none | \
cmp -n 752568 - u-boot.bin && \
echo "MATCH: u-boot.bin starts exactly at offset 0x2F000"
```

The result was:

``` text
MATCH: u-boot.bin starts exactly at offset 0x2F000
```

So I confirmed the internal structure of the combined file:
...
This test only proves the internal layout of the generated U-Boot file.
It does not prove the exact physical SD offset that the TH1520 BootROM
expects.

------------------------------------------------------------------------

## 23. Inspecting the beginning of the WIC image

I then looked more closely at the WIC disk layout.

The generated WIC starts as a standard GPT disk image:

``` text
Sector 0  → Protective MBR
Sector 1  → GPT header
Sector 2+ → GPT partition entries
```

I also checked the WIC implementation being used by the build.

It comes from:

``` text
openembedded-core/meta/recipes-support/wic/wic_0.3.1.bb
```

and its source uses `parted` to create the GPT partition table.

The WKS parser supports normal partition table selection such as:

``` text
--ptable gpt
```

but the WIC version I inspected did not expose an option equivalent to
the `gpt-location` mechanism that I later found in another
image-generation approach.

------------------------------------------------------------------------

## 24. Comparing with BeagleV-Ahead Buildroot work

While investigating how other projects construct a BeagleV-Ahead SD
image, I found Buildroot work using `genimage`.
...
This was interesting because `u-boot-with-spl.bin` is explicitly
included in the disk image outside the normal partition table.

The Yocto `beaglev-ahead.wks` I had used did not contain an equivalent
raw-copy instruction for `u-boot-with-spl.bin`.
